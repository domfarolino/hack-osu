...
module.exports = class Directions {
  ...
  async apiRequest() {
    try {
      const userCoordinate = this._userCoordinate.toStringBackwards();
      const destinationCoordinates = this._destinationCoordinates.map(object => `${object.toStringBackwards()};`).reduce((a, b) => a + b);
      const options = {
        uri: `https://route.arcgis.com/arcgis/rest/services/World/ClosestFacility/NAServer/ClosestFacility_World/solveClosestFacility?token=${this._token}&incidents=${userCoordinate}&facilities=${destinationCoordinates}&returnCFRoutes=true&f=json&defaultTargetFacilityCount=${this._maxTargetDestinations}`,
        json: true
      };
